...
import { SymbolData, ExpiryData, AIReadyData, LevelHistory } from '../types';
...
const HISTORY_URL = 'https://raw.githubusercontent.com/pitgian/quant-options-agent/master/data/level_history.json';
...
export interface VercelOptionsData {
  ...
  generated: string;
  /** Dynamic tolerances based on VIX (optional for backward compatibility) */
  dynamic_tolerances?: DynamicTolerances;
...
// ============================================================================
// LEVEL HISTORY FETCHING
// ============================================================================

/**
 * Fetches level history data from GitHub Raw URL.
 * This data contains the last 10 snapshots per symbol for level evolution tracking.
 *
 * The fetch is non-blocking — if it fails, null is returned and the app
 * works normally without evolution data.
 *
 * @returns Promise resolving to LevelHistory or null on error
 *
 * @example
 * ```typescript
 * const history = await fetchLevelHistory();
 * if (history) {
 *   const spySnapshots = history.level_history['SPY'];
 *   console.log('SPY snapshots:', spySnapshots?.length ?? 0);
 * }
 * ```
 */
export async function fetchLevelHistory(): Promise<LevelHistory | null> {
  try {
    console.log('[vercelDataService] Fetching level history from GitHub Raw...');
    ...
    if (!response.ok) {
      console.warn('[vercelDataService] Level history fetch failed:', response.status);
      return null;
    }

    const data: LevelHistory = await response.json();

    // Validate required fields
    if (!data.level_history) {
      console.warn('[vercelDataService] Invalid level history structure');
      return null;
    }

    const symbols = Object.keys(data.level_history);
    console.log('[vercelDataService] Level history loaded for symbols:', symbols.join(', '),
      '| Snapshots:', symbols.map(s => `${s}:${data.level_history[s]?.length ?? 0}`).join(', '));

    return data;
  } catch (error) {
    console.warn('[vercelDataService] Error fetching level history:', error);
    return null;
  }
}
