...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False, return_samples=False):
    with torch.no_grad():
        ...
        preds = z.cpu().numpy()
        if return_samples:
            # Return the full (batch, sample_count, seq_len, feat) array so the
            # caller can derive percentile bands (p10/p50/p90). When False, keep
            # the legacy contract of averaging the samples (used by train_adapter
            # baseline denoise and benchmark_sample_count).
            return preds
...
class KronosPredictor:
    ...
    def generate(self, x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, return_samples=False):
        ...
        if return_samples:
            # preds: (batch, sample_count, total_seq_len, d_in) -> keep last pred_len
    ...
    def _apply_adapter(self, preds, df, pred_len, batch=False, verbose=False, per_sample=False):
        """
        ...
        `preds` shapes:
          * single:    (pred_len, 6)
          * batch:     (B, pred_len, 6)
          * per_sample:(sample_count, pred_len, 6)  — treated as a batch over
            samples with identical covariates replicated across all draws.
        ...
        try:
            if per_sample:
                # preds: (sample_count, pred_len, 6) in normalized space.
                # Covariates are constant for the forecast, so they are
                # replicated across every Monte Carlo draw and the MLP is
                ...
                with torch.no_grad():
                    ...
                    sk = torch.tensor([[skew]] * S, dtype=torch.float32, device=self.device)
                    pc = torch.tensor([[pcr]] * S, dtype=torch.float32, device=self.device)
                    gx = torch.tensor([[gex]] * S, dtype=torch.float32, device=self.device)
                    residual = self.adapter(base, sk, pc, gx, pred_len).cpu().numpy()
                preds = preds + residual
                diag["applied"] = True
                diag["residual_norm"] = float(np.linalg.norm(residual) / max(1, S))
                if verbose:
                    print(f"Applied unified Residual Covariate Adapter per-sample "
                          f"(pred_len={pred_len}, samples={S}, skew={skew:.4f}, "
                          f"pcr={pcr:.4f}, gex={gex:.3f}B, "
                          f"mean|res|={diag['residual_norm']:.4f})")
            elif batch:
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True, return_samples=False):
        ...
        if return_samples:
            # preds: (1, sample_count, pred_len, 6) normalized
            samples_norm = preds.squeeze(0)  # (sample_count, pred_len, 6)
            # Apply the covariate adapter independently to every Monte Carlo
            # draw so the uncertainty reflects the options regime too. The
            # median draw drives the adapter diagnostics surfaced in the JSON.
            samples_norm, self.adapter_diag = self._apply_adapter(
                samples_norm, df, pred_len, verbose=verbose, per_sample=True)
            # De-normalize in real space (broadcast over the sample axis).
            samples = samples_norm * (x_std[:6] + 1e-5) + x_mean[:6]
            # Central estimate = per-element median (p50): consistent with the
            # p10/p90 band and more robust to skewed sample distributions than
            # the arithmetic mean.
            central = np.median(samples, axis=0)
            pred_df = pd.DataFrame(central, columns=self.price_cols + [self.vol_col, self.amt_vol], index=y_timestamp)
            return pred_df, samples
