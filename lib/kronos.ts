...
export interface ScaledKronosCandle extends KronosPredictedCandle {
  ...
  label: string;
  rawVolume: number;
  // Confidence band scaled into display space (undefined when the source
  // snapshot predates the Monte Carlo band — the chart degrades gracefully).
  close_p10?: number;
  close_p90?: number;
  high_p10?: number;
  high_p90?: number;
...
export interface ActiveKronosForecast {
  ...
  /** Central (p50) horizon range in display space — the "most likely" band.
   *  Undefined for snapshots predating the Monte Carlo band. */
  expectedHighP50?: number;
  expectedLowP50?: number;
  /** Whether the source snapshot carries per-candle percentile fields. */
...
export function getActiveKronosForecast(
  ...
  const candles: ScaledKronosCandle[] = sliced.map((c, idx) => {
    ...
    // Scale the MC band exactly like OHLC so the band stays aligned with the
    // re-anchored trajectory. Omit (undefined) when the snapshot lacks it.
    const scaleBand = (v: number | undefined) =>
      v != null ? v * scaleRatio * multiplier : undefined;
    return {
      ...c,
      open,
      high,
      low,
      close,
      close_p10: scaleBand(c.close_p10),
      close_p90: scaleBand(c.close_p90),
      high_p10: scaleBand(c.high_p10),
      high_p90: scaleBand(c.high_p90),
      low_p10: scaleBand(c.low_p10),
      low_p90: scaleBand(c.low_p90),
      changePct,
  ...
  // Central (p50) horizon range, scaled like everything else. Undefined when
  // the snapshot predates the MC band.
  const expectedHighP50 = activeData.expected_high_p50 != null
    ? activeData.expected_high_p50 * scaleRatio * multiplier
    : undefined;
  const expectedLowP50 = activeData.expected_low_p50 != null
    ? activeData.expected_low_p50 * scaleRatio * multiplier
    : undefined;
  const hasConfidenceBand = candles.length > 0
    && candles.every(c => c.close_p10 != null && c.close_p90 != null);
