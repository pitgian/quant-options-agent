...
The Python <-> TS **parity tests** (`utils/parity.test.ts`) go one step further:
they consume fixtures generated by the Python pipeline and assert that both
implementations produce numerically identical results. See
[`docs/python-ts-parity.md`](docs/python-ts-parity.md) for the full picture of
what is shared and what is intentionally divergent.
...
> **Data source:** the SPA reads generated JSON from the repo `data` branch
> (raw GitHub URL). No Gist or extra env vars are required — a cache-busting
> query keeps the CDN response fresh.
...
(`TWELVEDATA_API_KEY`, …) — see `.github/workflows/`.
